...
-- 注意：此 SQL 文件已废弃
-- ============================================
-- 系统现在使用 GORM AutoMigrate 自动管理数据库表结构。
-- 请运行 `go run cmd/lumenim/main.go migrate` 来初始化数据库。
-- 此文件仅保留作为表结构的参考文档。
-- ============================================
...
(
    ...
    `session_id`  int unsigned     NOT NULL DEFAULT '0' COMMENT '会话ID（关联成对的两条记录）',
    `talk_mode`   tinyint unsigned NOT NULL DEFAULT '1' COMMENT '聊天类型[1:私信;2:群聊;]',
    `user_id`     int unsigned     NOT NULL DEFAULT '0' COMMENT '用户ID',
    `receiver_id` int unsigned     NOT NULL COMMENT '接收者ID（用户ID 或 群ID）',
    ...
    `updated_at`  datetime         NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_user_id_receiver_id_talk_mode` (`user_id`, `receiver_id`, `talk_mode`) USING BTREE,
...
(
    ...
    `session_id`  int unsigned     NOT NULL DEFAULT '0' COMMENT '会话ID（关联talk_session.id）',
    `msg_type`    int unsigned     NOT NULL DEFAULT '1' COMMENT '消息类型',
    `user_id`     int unsigned     NOT NULL COMMENT '消息所属用户ID',
    `receiver_id` int unsigned     NOT NULL COMMENT '对方用户ID',
    `from_id`     int unsigned     NOT NULL COMMENT '消息发送者ID',
    `is_revoked`  tinyint unsigned NOT NULL DEFAULT '2' COMMENT '是否撤回[1:是;2:否;]',
    `is_deleted`  tinyint unsigned NOT NULL DEFAULT '2' COMMENT '是否删除[1:是;2:否;]',
    `extra`       json             NOT NULL COMMENT '消息扩展字段',
    ...
    KEY `idx_session_id` (`session_id`) USING BTREE,
    KEY `idx_user_id_receiver_id` (`user_id`, `receiver_id`) USING BTREE,
