...
func (s *EmoticonService) RemoveUserSysEmoticon(uid int, emoticonId int) error {
	...
	return s.Source.Db().Model(&model.UsersEmoticon{}).Where("user_id = ?", uid).Update("emoticon_ids", strings.Join(items, ",")).Error
}
...
func (s *EmoticonService) AddUserSysEmoticon(uid int, emoticonId int) error {
	...
	return s.Source.Db().Model(&model.UsersEmoticon{}).Where("user_id = ?", uid).Update("emoticon_ids", sliceutil.ToIds(ids)).Error
}
