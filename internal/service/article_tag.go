...
func (s *ArticleTagService) Update(ctx context.Context, uid int, tagId int, tag string) error {
	return s.Source.Db().WithContext(ctx).Model(&model.ArticleTag{}).Where("id = ? and user_id = ?", tagId, uid).UpdateColumn("tag_name", tag).Error
}
...
func (s *ArticleTagService) Delete(ctx context.Context, uid int, tagId int) error {
	...
	var num int64
	if err := db.Model(&model.Article{}).Where("user_id = ? and FIND_IN_SET(?,tags_id)", uid, tagId).Count(&num).Error; err != nil {
		return err
	}
...
func (s *ArticleTagService) List(ctx context.Context, uid int) ([]*model.TagItem, error) {
	...
	err := db.Model(&model.ArticleTag{}).Select("id", "tag_name").Where("user_id = ?", uid).Scan(&items).Error
	if err != nil {
		return nil, err
	}
	...
	for _, item := range items {
		var num int64
		if err := db.Model(&model.Article{}).Where("user_id = ? and status = 1 and FIND_IN_SET(?,tags_id)", uid, item.Id).Count(&num).Error; err == nil {
			item.Count = int(num)
		}
	}
