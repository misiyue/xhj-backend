...
type IGroupService interface {
	...
	List(ctx context.Context, userId int) ([]*model.GroupItem, error)
}
...
func (g *GroupService) Create(ctx context.Context, opt *GroupCreateOpt) (int, error) {
	var (
		...
		record   *model.TalkGroupMessage // Moved record outside to push message after transaction
	)
	...
	group := &model.Group{
		...
		IsOvert:       model.No,
		IsMute:        model.No,
		IsAllowInvite: 1, // 与 GroupDetailResponse：1 允许群内互加
	}
	...
	// Push messages only after transaction succeeded
	_ = g.PushMessage.MultiPush(ctx, entity.ImTopicChat, []*entity.SubscribeMessage{
		...
		// Push the group creation message so frontend can update chat list
		{
...
func (g *GroupService) Invite(ctx context.Context, opt *GroupInviteOpt) error {
	...
	db.Select("id", "user_id", "is_delete").Where("user_id in ? and receiver_id = ? and talk_mode = ?", opt.MemberIds, opt.GroupId, entity.ChatGroupMode).Find(&talkList)
...
func (g *GroupService) List(ctx context.Context, userId int) ([]*model.GroupItem, error) {
	db := g.Source.Db().WithContext(ctx)

	tx := db.Model(&model.GroupMember{})
	...
	query := db.Model(&model.TalkSession{})
	query.Select("receiver_id,is_disturb")
	query.Where("user_id = ? and talk_mode = ? and receiver_id in ?", userId, entity.ChatGroupMode, ids)
