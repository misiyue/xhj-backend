...
func (c *ContactGroupService) Delete(ctx context.Context, id int, uid int) error {
	return c.ContactGroupRepo.Txx(ctx, func(tx *gorm.DB) error {
		...
		return tx.Model(&model.Contact{}).Where("user_id = ? and group_id = ?", uid, id).UpdateColumn("group_id", 0).Error
	})
}
...
func (c *ContactGroupService) GetUserGroup(ctx context.Context, uid int) ([]*model.ContactGroup, error) {
	...
	err := c.Source.Db().WithContext(ctx).Model(&model.ContactGroup{}).Where("user_id = ?", uid).Order("sort asc").Scan(&items).Error
	if err != nil {
		return nil, err
	}
