...
func (s *ArticleClassService) Delete(ctx context.Context, uid, cid int) error {
	...
	var num int64
	if err := s.Source.Db().WithContext(ctx).Model(&model.Article{}).Where("user_id = ? and class_id = ?", uid, cid).Count(&num).Error; err != nil {
		return err
	}
...
func (s *ArticleClassService) Sort(ctx context.Context, uid int32, ids []int32) error {
	...
	return s.Source.Db().Transaction(func(tx *gorm.DB) error {
		for k, v := range sortItems {
			err = tx.Model(&model.ArticleClass{}).Where("id = ?", k).Update("sort", v).Error
