...
func (g *GroupVoteService) Submit(ctx context.Context, opt *GroupVoteSubmitOpt) error {
	...
	var count int64
	db.Model(&model.GroupVoteAnswer{}).Where("vote_id = ? and user_id = ?", opt.VoteId, opt.UserId).Count(&count)
	if count > 0 {
	...
	err = g.Source.Db().Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&model.GroupVote{}).Where("id = ?", voteInfo.Id).Updates(data).Error; err != nil {
			return err
		}
