...
func (o *Organize) List() ([]*UserInfo, error) {
	...
	tx := o.Repo.Db.Model(&model.Organize{})
	tx.Select([]string{
...
func (o *Organize) GetMemberIds(ctx context.Context) ([]int64, error) {
	var ids []int64
	if err := o.Repo.Db.WithContext(ctx).Model(&model.Organize{}).Pluck("user_id", &ids).Error; err != nil {
		return nil, err
	}
...
func (o *Organize) DepartmentGroupCount(ctx context.Context) ([]*GroupCount, error) {
	var resp []*GroupCount
	err := o.Db.WithContext(ctx).Model(&model.Organize{}).Select("dept_id, count(*) as count").Where("dept_id > 0").Group("dept_id").Scan(&resp).Error
	if err != nil {
		return nil, err
	}
