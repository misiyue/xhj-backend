...
func (t *GroupVote) SetVoteAnswerUser(ctx context.Context, vid int) ([]int, error) {
	...
	err := t.Repo.Db.WithContext(ctx).Model(&model.GroupVoteAnswer{}).Where("vote_id = ?", vid).Pluck("user_id", &uids).Error
...
func (t *GroupVote) SetVoteStatistics(ctx context.Context, vid int) (*VoteStatistics, error) {
	...
	if err := tx.Model(&model.GroupVote{}).First(&vote, vid).Error; err != nil {
		return nil, err
	}
	...
	err := tx.Model(&model.GroupVoteAnswer{}).Where("vote_id = ?", vid).Pluck("option", &options).Error
	if err != nil {
		return nil, err
	}
...
func (t *GroupVote) FindAllAnsweredUserList(ctx context.Context, viteId int) ([]model.GroupVoteAnswer, error) {
	...
	err := t.Repo.Db.Model(&model.GroupVoteAnswer{}).Where("vote_id = ?", viteId).Scan(&items).Error
	if err == nil {
		return items, nil
	}
