...
type Article struct {
	...
	UserId     int          `gorm:"column:user_id;index:idx_article_user_status_class,priority:1" json:"user_id"`   // 用户ID
	ClassId    int          `gorm:"column:class_id;index:idx_article_user_status_class,priority:3" json:"class_id"` // 分类ID
	TagsId     string       `gorm:"column:tags_id;type:varchar(255);" json:"tags_id"`                               // 笔记关联标签
	Title      string       `gorm:"column:title;type:varchar(255);" json:"title"`                                   // 文章标题
	Abstract   string       `gorm:"column:abstract;type:varchar(512);" json:"abstract"`                             // 文章摘要
	Image      string       `gorm:"column:image;type:varchar(255);" json:"image"`                                   // 文章首图
	IsAsterisk int          `gorm:"column:is_asterisk;" json:"is_asterisk"`                                         // 是否星标文章[0:否;1:是;]
	Status     int          `gorm:"column:status;index:idx_article_user_status_class,priority:2" json:"status"`     // 笔记状态[1:正常;2:已删除;]
	MdContent  string       `gorm:"column:md_content;type:text;" json:"md_content"`                                 // Markdown 内容
