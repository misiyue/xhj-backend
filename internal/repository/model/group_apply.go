...
type GroupApply struct {
	...
	GroupId   int       `gorm:"column:group_id;index:idx_group_apply_group_status_updated,priority:1" json:"group_id"`     // 群组ID
	UserId    int       `gorm:"column:user_id;" json:"user_id"`                                                            // 用户ID
	Status    int       `gorm:"column:status;index:idx_group_apply_group_status_updated,priority:2" json:"status"`         // 申请状态
	Remark    string    `gorm:"column:remark;" json:"remark"`                                                              // 备注信息
	Reason    string    `gorm:"column:reason;" json:"reason"`                                                              // 拒绝原因
	CreatedAt time.Time `gorm:"column:created_at;" json:"created_at"`                                                      // 创建时间
	UpdatedAt time.Time `gorm:"column:updated_at;index:idx_group_apply_group_status_updated,priority:3" json:"updated_at"` // 更新时间
}
