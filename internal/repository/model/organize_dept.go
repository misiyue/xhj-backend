...
type OrganizeDept struct {
	...
	ParentId  int       `gorm:"column:parent_id;index:idx_dept_deleted_parent_order,priority:2" json:"parent_id"`   // 父部门id
	Ancestors string    `gorm:"column:ancestors;" json:"ancestors"`                                                 // 祖级列表
	DeptName  string    `gorm:"column:dept_name;" json:"dept_name"`                                                 // 部门名称
	OrderNum  int       `gorm:"column:order_num;index:idx_dept_deleted_parent_order,priority:3" json:"order_num"`   // 显示顺序
	...
	IsDeleted int       `gorm:"column:is_deleted;index:idx_dept_deleted_parent_order,priority:1" json:"is_deleted"` // 是否删除[1:否;2:是]
