...
func RegisterGroupHandler(r gin.IRoutes, interceptor interface {
	...
	r.POST("/api/v1/group/allowInvite", interceptor.Do(func(ctx *gin.Context) (any, error) {
		var in GroupAllowInviteRequest
		if err := interceptor.ShouldProto(ctx, &in); err != nil {
			return nil, err
		}

		return handler.AllowInvite(ctx.Request.Context(), &in)
	}))
